...
class ConsumirReservaServiceTest {
    ...
    @InjectMocks
    private ConsumirReservaService service;
    ...
    void deveConsumirReservaComSucesso() {
        ...
        when(commandGateway.send(any(AlmoxarifadoConsumirEstoqueCommand.class)))
                .thenReturn(CompletableFuture.completedFuture(null));

        ConsumirReservaCommand command = new ConsumirReservaCommand(pedidoId);

        CompletableFuture<Void> resultado = service.consumirReservas(command);

        resultado.join();

        verify(commandGateway, times(1)).send(argThat(cmd ->
                cmd != null && ((AlmoxarifadoConsumirEstoqueCommand)cmd).getId().equals(almoxId1)
        ));
        verify(commandGateway, times(1)).send(argThat(cmd ->
                cmd != null && ((AlmoxarifadoConsumirEstoqueCommand)cmd).getId().equals(almoxId2)
        ));
    }

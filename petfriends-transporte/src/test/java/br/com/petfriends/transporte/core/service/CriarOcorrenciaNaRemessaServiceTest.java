...
class CriarOcorrenciaNaRemessaServiceTest {
    @Mock
    private CommandGateway commandGateway;

    @InjectMocks
    private CriarOcorrenciaNaRemessaService service;

    @Test
    void deveEnviarComandoParaOGateway() {
        CriarOcorrenciaRemessaCommand command = mock(CriarOcorrenciaRemessaCommand.class);
        CompletableFuture<Void> future = CompletableFuture.completedFuture(null);
        when(commandGateway.send(command)).thenReturn((CompletableFuture)future);

        CompletableFuture<Void> resultado = service.criarOcorrencia(command);
